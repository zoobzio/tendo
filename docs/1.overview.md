...
Tensors wrap a Storage backend (CPU or CUDA) with shape metadata. Operations dispatch to the appropriate device. Events flow through capitan for observability.
...
**[Observability](4.cookbook/2.observability.md)** - Every operation emits capitan signals. Hook listeners for logging, profiling, or debugging.
...
ctx := context.Background() // inference mode by default
output, err := forward.Run(ctx, input)
```

Inference vs training mode is context-driven. Dropout passes through during inference (default), applies during training mode.
