...
## Context and Training Mode

### Training vs Inference

```go
// Training mode: dropout applies
trainCtx := tendo.WithTraining(context.Background())
output, _ := network.Run(trainCtx, input)

// Inference mode: dropout passes through
inferCtx := tendo.WithInference(context.Background())
output, _ := network.Run(inferCtx, input)
...
// Inference: dropout bypassed
inferOut, _ := forward.Run(tendo.WithInference(ctx), input)
```
...
// Training
trainCtx := tendo.WithTraining(context.Background())
logits, err := model.Forward(trainCtx, batch)

// Inference
inferCtx := tendo.WithInference(context.Background())
predictions, err := model.Forward(inferCtx, input)
```
